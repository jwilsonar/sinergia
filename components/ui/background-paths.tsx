...
function FloatingPaths({
  ...
  return (
    <div className="absolute inset-0 pointer-events-none">
      <svg
        className={cn(
          "w-full h-full min-h-full text-slate-950 dark:text-white",
          svgClassName,
        )}
        viewBox="0 0 696 316"
...
/** Solo las rutas animadas; úsalo como capa absoluta detrás del contenido del hero. */
export function BackgroundPathsLayer({
  className,
  svgClassName,
}: {
  className?: string;
  /** Color de trazo (`currentColor`), p. ej. tema oscuro: `text-[#F0EFE8]` */
  svgClassName?: string;
}) {
  return (
    <div
      ...
      aria-hidden
    >
      <div className="absolute inset-0 min-h-full origin-center scale-[1.35]">
        <FloatingPaths position={1} svgClassName={svgClassName} />
        <FloatingPaths position={-1} svgClassName={svgClassName} />
      </div>
    </div>
  );
